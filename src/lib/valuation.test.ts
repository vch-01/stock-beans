...
describe('valuation helpers', () => {
  ...
  it('classifies stocks as undervalued, fair, or overvalued', () => {
    expect(deriveStockValuation(buildStock({ price: 100, allTimeHigh: 200, forwardPE: 10, valueScore: 90 }))).toBe('undervalued')
    expect(deriveStockValuation(buildStock({ price: 100, allTimeHigh: 200, forwardPE: 10, valueScore: 35 }))).toBe('overvalued')
    expect(deriveStockValuation(buildStock({ price: 100, allTimeHigh: 200, forwardPE: 10, valueScore: 50 }))).toBe('fair')
  })

  it('enriches stock metrics with earnings yield, score, and valuation', () => {
    const enriched = enrichStockMetrics(buildStock({ price: 100, allTimeHigh: 200, forwardPE: 10 }))
